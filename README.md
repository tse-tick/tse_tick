...
| `--parallel` | Parallel worker processes for per-date ingest (default 1). Applies to `--period`, `--year`, and `--flat`; not to `--filter-csv` event windows. Capped by the machine's logical cores AND available RAM (each worker holds a whole day's frame). |
...
| Max ZIP entries | 5 |
| Parallel worker cap | Logical cores, then a RAM budget (70% of available; each worker holds one day's frame) |
